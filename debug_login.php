...
try {
    ...
    if (empty($users)) {
        ...
        echo "<tr><th>ID</th><th>Email</th><th>Mot de passe (première partie)</th><th>Longueur hash</th><th>Type (bcrypt?)</th></tr>";
        foreach ($users as $user) {
            ...
            echo "<td>" . htmlspecialchars(substr($hash, 0, 20) . '...') . "</td>";
    ...
    if (!empty($users)) {
        ...
        $testPassword = "test"; // À changer selon votre mot de passe de test
        
        echo "<p>Email de test : " . htmlspecialchars($testEmail) . "</p>";
        echo "<p>Mot de passe de test : " . htmlspecialchars($testPassword) . "</p>";
        ...
        echo "<p>Résultat password_verify : " . ($verifyResult ? "VRAI" : "FAUX") . "</p>";
        
        if ($testPassword === $hash) {
            echo "<p>Comparaison directe : VRAI (mot de passe non hashé)</p>";
        } else {
            echo "<p>Comparaison directe : FAUX (mot de passe hashé)</p>";
        }
    }
    
    echo "<h2>Créer un utilisateur de test :</h2>";
